...
| Capture strategy                    | Three layers: SDK (primary) + `chrome.webRequest` (observation) + `chrome.debugger` (armed by the user in the popup; the permission itself is required, Chrome refuses it as optional). Each covers the others' blind spots. |
...
4. Declare the MV3 permissions in `wxt.config.ts`: `tabCapture`, `offscreen`, `storage`, `webRequest` as required permissions; `debugger` as a required permission too — Chrome drops it from `optional_permissions` at load and refuses every runtime request for it (`aidd_docs/memory/architecture.md:78`), so what the user arms in the popup is the session, not the grant. **Host permissions are still open**: capture is scoped to user-designated domains, so requesting them per domain through `optional_host_permissions` fits the model better than a blanket install-time grant — verify that `webRequest` behaves as expected under an optional grant before committing.
