...
Overrides the global "authored artifacts in English" rule, for this project only.

- **Every written deliverable is in French:** briefs, plans, specifications, diagnostic reports, project memory, task documents, and user-facing documentation.
- **Code stays in English:** identifiers, code comments, commit messages, and pull request titles. This repository ships a public npm SDK and a Chrome Web Store extension; its history and source must stay readable by an external contributor.
- **The unit is the document, not the edit.** A document keeps the language it was written in; editing an English file does not switch it to French mid-page. Only new documents are born in French. `aidd_docs/memory/` predates this rule and stays English until someone decides to translate it.
