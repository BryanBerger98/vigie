...
**Last updated: 2026-08-13 · Applies to the disclosure wording shipped as version 2.**
...
keys. Response bodies are captured only while deep capture is on: it stays off until you start it
from the popup, and it then records the text of the responses a watched tab receives, up to 256 kB
each.
...
That report carries what the capture carries, request and response headers included, plus the
response bodies deep capture recorded if it was on. **Authentication tokens, session cookies, API
keys and whatever an API answered are therefore in it.** Read a report before sending it somewhere
you would not send a session cookie.
...
The agreement is tied to what is captured, not to the extension. If a future version captures
something this page does not describe, the disclosure changes, capture stops, and Vigie asks again.

Translating that disclosure asks nothing of anyone: the agreement is about the capture, and the
capture has not changed.
...
| `debugger` | Reading response bodies, and only while you have deep capture on. Chrome requires it in the manifest, so the warning is shown at install even if you never start the layer; nothing attaches until you do, and Chrome shows a banner on every tab while it runs |
